...
exposed_area_of_wing = wing_span*wing_chord - (fuselage_diameter*wing_chord);
...
for R = 1:length(v_infinity)
    dynamic_pressure(R) = 0.5*rho*v_infinity(R)^2;
    
    reynolds_numbers_wing_tail_fuselage = [
        (rho*v_infinity(R)*wing_chord)/mu;
        (rho*v_infinity(R)*tail_chord)/mu;
        (rho*v_infinity(R)*fuselage_length)/mu
        ];
    ...
    total_drag = (total_parasitic_drag_coefficient*dynamic_pressure(R)*reference_area)+(lift_of_plane_in_newtons^2 / (dynamic_pressure(R)*pi*wing_span^2*e));
    
    lift_to_drag_ratio(R) = lift_of_plane_in_newtons/total_drag;
end

[lift_to_drag_ratio_max, index] = max(lift_to_drag_ratio) % answer
corresponding_v_infinity = v_infinity(index) % answer
...
corresponding_lift_coefficient = lift_of_plane_in_newtons/(dynamic_pressure(index)*exposed_area_of_wing) % answer
corresponding_Re = rho*corresponding_v_infinity*wing_chord/mu % answer
